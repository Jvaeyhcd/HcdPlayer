...
#import "CDMovieGLView.h"

NSString * const CDMovieParameterMinBufferedDuration = @"CDMovieParameterMinBufferedDuration";
NSString * const CDMovieParameterMaxBufferedDuration = @"CDMovieParameterMaxBufferedDuration";
NSString * const CDMovieParameterDisableDeinterlacing = @"CDMovieParameterDisableDeinterlacing";
...
@property (readwrite, strong) CDArtworkFrame *artworkFrame;
...
+ (id)movieViewControllerWithContentPath:(NSString *)path
    ...
    return [[CDVideoViewController alloc] initWithContentPath:path canUseAudio:canUseAudio parameters:parameters];
}

- (id)initWithContentPath:(NSString *)path
              canUseAudio:(BOOL)canUseAudio
               parameters:(NSDictionary *)parameters {
...
//        self.wantsFullScreenLayout = YES;
        ...
        CDVideoDecodeType type = CDVideoDecodeTypeVideo;
        if (canUseAudio) {
            type |= CDVideoDecodeTypeAudio;
        }
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            [decoder setupVideoFrameFormat:CDVideoFrameFormatYUV];
            [decoder setUseHardwareDecompressor:YES];
...
- (void)setMovieDecoder:(CDPlayerDecoder *)decoder
...
//    LoggerStream(2, @"setMovieDecoder");
    ...
    if (!error && decoder) {
        ...
        _decoder        = decoder;
        _dispatchQueue  = dispatch_queue_create("CDMovie", DISPATCH_QUEUE_SERIAL);
        ...
        if (_parameters.count) {
            ...
            val = [_parameters valueForKey: CDMovieParameterMinBufferedDuration];
            if ([val isKindOfClass:[NSNumber class]])
                _minBufferedDuration = [val floatValue];
            
            val = [_parameters valueForKey: CDMovieParameterMaxBufferedDuration];
            if ([val isKindOfClass:[NSNumber class]])
                _maxBufferedDuration = [val floatValue];
            
            val = [_parameters valueForKey: CDMovieParameterDisableDeinterlacing];
            if ([val isKindOfClass:[NSNumber class]])
...
- (void)setupPresentView {
    ...
    if (_decoder.validVideo) {
        _glView = [[CDMovieGLView alloc] initWithFrame:bounds decoder:_decoder];
    }
    
    if (!_glView) {
        
//        LoggerVideo(0, @"fallback to use RGB video frame and UIKit");
        [_decoder setupVideoFrameFormat:CDVideoFrameFormatRGB];
            ...
            if (!_currentAudioFrame) {
                ...
                @synchronized(_audioFrames) {
                    ...
                    if (count > 0) {
                        ...
                        CDAudioFrame *frame = _audioFrames[0];
...
- (BOOL) addFrames: (NSArray *)frames
{
    ...
    if (_decoder.validAudio) {
        ...
        @synchronized(_audioFrames) {
            ...
            for (CDPlayerFrame *frame in frames)
                if (frame.type == CDPlayerFrameTypeAudio) {
        ...
        if (!_decoder.validVideo) {
            ...
            for (CDPlayerFrame *frame in frames)
                if (frame.type == CDPlayerFrameTypeArtwork)
                    self.artworkFrame = (CDArtworkFrame *)frame;
        }
    }
    ...
    if (_decoder.validSubtitles) {
        ...
        @synchronized(_subtitles) {
            ...
            for (CDPlayerFrame *frame in frames)
                if (frame.type == CDPlayerFrameTypeSubtitle) {
...
- (void) asyncDecodeFrames
{
    ...
    __weak typeof(self) weakSelf = self;
    __weak CDPlayerDecoder *weakDecoder = _decoder;
    ...
    dispatch_async(_dispatchQueue, ^{
        ...
        while (good) {
            ...
            @autoreleasepool {
                ...
                __strong CDPlayerDecoder *decoder = weakDecoder;
...
- (CGFloat)presentVideoFrame:(CDVideoFrame *)frame {
    
    if (_glView) {
        [_glView render:frame];
    } else {
        CDVideoFrameRGB *rgbFrame = (CDVideoFrameRGB *)frame;
...
- (void) presentSubtitles
{
    ...
    if ([self subtitleForPosition:_moviePosition
        ...
        if (actual.count) {
            ...
            for (CDSubtitleFrame *subtitle in actual.reverseObjectEnumerator) {
                    ...
                    outdated: (NSArray **) pOutdated
{
    ...
    for (CDSubtitleFrame *subtitle in _subtitles) {
