...
@property (nonatomic, strong) HCDPlayerView *view;
@property (nonatomic, strong) HCDPlayerDecoder *decoder;
@property (nonatomic, strong) HCDPlayerAudioManager *audio;

@property (nonatomic, strong) NSMutableArray *vframes;
@property (nonatomic, strong) NSMutableArray *aframes;
@property (nonatomic, strong) HCDPlayerAudioFrame *playingAudioFrame;
...
- (void)initVars {
    self.minBufferDuration = HCDPlayerMinBufferDuration;
    self.maxBufferDuration = HCDPlayerMaxBufferDuration;
...
- (void)initView {
    HCDPlayerView *v = [[HCDPlayerView alloc] init];
    self.view = v;
}

- (void)initDecoder {
    self.decoder = [[HCDPlayerDecoder alloc] init];
}

- (void)initAudio {
    self.audio = [[HCDPlayerAudioManager alloc] init];
}
...
- (void)open:(NSString *)url {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:HCDPlayerNotificationOpened object:strongSelf];
        });
    });
}
...
- (void)close {
    if (!self.opened && !self.opening) {
        [[NSNotificationCenter defaultCenter] postNotificationName:HCDPlayerNotificationClosed object:self];
        return;
    }
    ...
    dispatch_source_set_event_handler(timer, ^{
        ...
        if ([strongSelf.audio close:&errors]) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:HCDPlayerNotificationClosed object:strongSelf];
        } else {
...
- (void)readFrame {
    ...
    while (self.playing && !self.decoder.isEOF && !self.requestSeek
        ...
        @autoreleasepool {
            ...
            {
                for (HCDPlayerFrame *f in fs) {
                    if (f.type == kHCDPlayerFrameTypeVideo) {
            ...
            {
                for (HCDPlayerFrame *f in fs) {
                    if (f.type == kHCDPlayerFrameTypeAudio) {
...
- (void)render {
    ...
    if (noframes && eof) {
        [self pause];
        [[NSNotificationCenter defaultCenter] postNotificationName:HCDPlayerNotificationEOF object:self];
        return;
    }
    
    if (noframes && !self.notifiedBufferStart) {
        self.notifiedBufferStart = YES;
        NSDictionary *userInfo = @{ HCDPlayerNotificationBufferStateKey : @(self.notifiedBufferStart) };
        [[NSNotificationCenter defaultCenter] postNotificationName:HCDPlayerNotificationBufferStateChanged object:self userInfo:userInfo];
    ...
    if (self.decoder.hasPicture && self.vframes.count > 0) {
        HCDPlayerVideoFrame *frame = self.vframes[0];
...
/*
 * For audioUnitRenderCallback, (HCDPlayerAudioManagerFrameReaderBlock)readFrameBlock
 */
            ...
            if (self.playingAudioFrame == nil) {
                {
                    ...
                    if (timeout == 0) {
                        HCDPlayerAudioFrame *frame = self.aframes[0];
                        if (self.decoder.hasVideo) {
...
- (void)handleError:(NSError *)error {
    if (error == nil) return;
    NSDictionary *userInfo = @{ HCDPlayerNotificationErrorKey : error };
    [[NSNotificationCenter defaultCenter] postNotificationName:HCDPlayerNotificationError object:self userInfo:userInfo];
}
