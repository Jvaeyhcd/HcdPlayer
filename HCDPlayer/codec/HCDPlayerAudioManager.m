...
#import "HCDPlayerAudioManager.h"
...
@interface HCDPlayerAudioManager () {
    BOOL _registeredKVO;
    BOOL _opened;
    BOOL _closing;
...
@implementation HCDPlayerAudioManager
...
- (BOOL)open:(NSError **)error {
    ...
    if (![session setCategory:AVAudioSessionCategoryPlayback error:&rawError]) {
        [HCDPlayerUtils createError:error
                         withDomain:HCDPlayerErrorDomainAudioManager
                            andCode:HCDPlayerErrorCodeCannotSetAudioCategory
                         andMessage:[HCDPlayerUtils localizedString:@"HCD_PLAYER_STRINGS_CANNOT_SET_AUDIO_CATEGORY"]
    ...
    if (![session setActive:YES error:&rawError]) {
        [HCDPlayerUtils createError:error
                         withDomain:HCDPlayerErrorDomainAudioManager
                            andCode:HCDPlayerErrorCodeCannotSetAudioActive
                         andMessage:[HCDPlayerUtils localizedString:@"HCD_PLAYER_STRINGS_CANNOT_SET_AUDIO_ACTIVE"]
                        andRawError:rawError];
        return NO;
    }
    ...
    if (route.outputs.count == 0) {
        [HCDPlayerUtils createError:error
                         withDomain:HCDPlayerErrorDomainAudioManager
                            andCode:HCDPlayerErrorCodeNoAudioOuput
                         andMessage:[HCDPlayerUtils localizedString:@"HCD_PLAYER_STRINGS_NO_AUDIO_OUTPUT"]];
        return NO;
    }
    ...
    if (channels <= 0) {
        [HCDPlayerUtils createError:error
                         withDomain:HCDPlayerErrorDomainAudioManager
                            andCode:HCDPlayerErrorCodeNoAudioChannel
                         andMessage:[HCDPlayerUtils localizedString:@"HCD_PLAYER_STRINGS_NO_AUDIO_CHANNEL"]];
        return NO;
    }
    ...
    if (sampleRate <= 0) {
        [HCDPlayerUtils createError:error
                         withDomain:HCDPlayerErrorDomainAudioManager
                            andCode:HCDPlayerErrorCodeNoAudioSampleRate
                         andMessage:[HCDPlayerUtils localizedString:@"HCD_PLAYER_STRINGS_NO_AUDIO_SAMPLE_RATE"]];
        return NO;
    }
    ...
    if (volume < 0) {
        [HCDPlayerUtils createError:error
                         withDomain:HCDPlayerErrorDomainAudioManager
                            andCode:HCDPlayerErrorCodeNoAudioVolume
                         andMessage:[HCDPlayerUtils localizedString:@"HCD_PLAYER_STRINGS_NO_AUDIO_VOLUME"]];
        return NO;
    }
...
- (BOOL)initAudioUnitWithSampleRate:(double)sampleRate andRenderCallback:(AURenderCallback)renderCallback error:(NSError **)error {
    ...
    if (status != noErr) {
        ...
        [HCDPlayerUtils createError:error
                         withDomain:HCDPlayerErrorDomainAudioManager
                            andCode:HCDPlayerErrorCodeCannotCreateAudioComponent
                         andMessage:[HCDPlayerUtils localizedString:@"HCD_PLAYER_STRINGS_CANNOT_CREATE_AUDIO_UNIT"]
                        andRawError:rawError];
        return NO;
    }
    ...
    if (status != noErr) {
        ...
        [HCDPlayerUtils createError:error
                         withDomain:HCDPlayerErrorDomainAudioManager
                            andCode:HCDPlayerErrorCodeCannotGetAudioStreamDescription
                         andMessage:[HCDPlayerUtils localizedString:@"HCD_PLAYER_STRINGS_CANNOT_GET_AUDIO_STREAM_DESCRIPTION"]
                        andRawError:rawError];
        return NO;
    }
    ...
    if (status != noErr) {
        ...
        [HCDPlayerUtils createError:error
                         withDomain:HCDPlayerErrorDomainAudioManager
                            andCode:HCDPlayerErrorCodeCannotSetAudioRenderCallback
                         andMessage:[HCDPlayerUtils localizedString:@"HCD_PLAYER_STRINGS_CANNOT_SET_AUDIO_RENDER_CALLBACK"]
                        andRawError:rawError];
        return NO;
    }
    ...
    if (status != noErr) {
        ...
        [HCDPlayerUtils createError:error
                         withDomain:HCDPlayerErrorDomainAudioManager
                            andCode:HCDPlayerErrorCodeCannotInitAudioUnit
                         andMessage:[HCDPlayerUtils localizedString:@"HCD_PLAYER_STRINGS_CANNOT_INIT_AUDIO_UNIT"]
                        andRawError:rawError];
        return NO;
    }
    ...
    if (_opened) {
        ...
        if (status != noErr) {
            ...
            if (errs != nil) {
                ...
                [HCDPlayerUtils createError:&error
                                 withDomain:HCDPlayerErrorDomainAudioManager
                                    andCode:HCDPlayerErrorCodeCannotUninitAudioUnit
                                 andMessage:[HCDPlayerUtils localizedString:@"HCD_PLAYER_STRINGS_CANNOT_UNINIT_AUDIO_UNIT"]
                                andRawError:rawError];
        ...
        if (status != noErr) {
            ...
            if (errs != nil) {
                ...
                [HCDPlayerUtils createError:&error
                                 withDomain:HCDPlayerErrorDomainAudioManager
                                    andCode:HCDPlayerErrorCodeCannotDisposeAudioUnit
                                 andMessage:[HCDPlayerUtils localizedString:@"HCD_PLAYER_STRINGS_CANNOT_DISPOSE_AUDIO_UNIT"]
                                andRawError:rawError];
                [errs addObject:error];
            }
        }
        ...
        if (![session setActive:NO error:&error]) {
            ...
            if (errs != nil) {
                ...
                [HCDPlayerUtils createError:&error
                                 withDomain:HCDPlayerErrorDomainAudioManager
                                    andCode:HCDPlayerErrorCodeCannotDeactivateAudio
                                 andMessage:[HCDPlayerUtils localizedString:@"HCD_PLAYER_STRINGS_CANNOT_DEACTIVATE_AUDIO"]
                                andRawError:rawError];
                [errs addObject:error];
            }
        }
...
- (BOOL)play:(NSError **)error {
    if (_opened) {
        ...
        if (!_playing) {
            ...
            [HCDPlayerUtils createError:error
                             withDomain:HCDPlayerErrorDomainAudioManager
                                andCode:HCDPlayerErrorCodeCannotStartAudioUnit
                             andMessage:[HCDPlayerUtils localizedString:@"HCD_PLAYER_STRINGS_CANNOT_START_AUDIO_UNIT"]
                            andRawError:rawError];
        }
    }
    return _playing;
...
- (BOOL)pause:(NSError **)error {
    if (_playing) {
        ...
        if (_playing) {
            ...
            [HCDPlayerUtils createError:error
                             withDomain:HCDPlayerErrorDomainAudioManager
                                andCode:HCDPlayerErrorCodeCannotStopAudioUnit
                             andMessage:[HCDPlayerUtils localizedString:@"HCD_PLAYER_STRINGS_CANNOT_STOP_AUDIO_UNIT"]
                            andRawError:rawError];
        }
    }
    return !_playing;
...
static OSStatus audioUnitRenderCallback(void *inRefCon,
                                        ...
                                        UInt32 inNumberFrames,
                                        AudioBufferList *ioData) {
    HCDPlayerAudioManager *manager = (__bridge HCDPlayerAudioManager *)(inRefCon);
