...
static int interruptCallback(void *context) {
    ...
    if (g_bPrepareClose || dt > HCDPlayerIOTimeout) return 1;
    return 0;
}

@interface HCDPlayerDecoder () {
...
- (BOOL)open:(NSString *)url error:(NSError **)error {
    if (url == nil || url.length == 0) {
        [HCDPlayerUtils createError:error
                         withDomain:HCDPlayerErrorDomainDecoder
                            andCode:HCDPlayerErrorCodeInvalidURL
                         andMessage:[HCDPlayerUtils localizedString:@"HCD_PLAYER_STRINGS_INVALID_URL"]];
        return NO;
    }
    ...
    if (ret != 0) {
        ...
        [HCDPlayerUtils createError:error
                         withDomain:HCDPlayerErrorDomainDecoder
                            andCode:HCDPlayerErrorCodeCannotOpenInput
                         andMessage:[HCDPlayerUtils localizedString:@"HCD_PLAYER_STRINGS_CANNOT_OPEN_INPUT"]];
        return NO;
    }
    ...
    if (ret < 0) {
        ...
        [HCDPlayerUtils createError:error
                         withDomain:HCDPlayerErrorDomainDecoder
                            andCode:HCDPlayerErrorCodeCannotFindStreamInfo
                         andMessage:[HCDPlayerUtils localizedString:@"HCD_PLAYER_STRINGS_CANNOT_FIND_STREAM_INFO"]];
        return NO;
    }
    ...
    if (vstream >= 0 && vcodectx != NULL) {
        if (vcodectx->pix_fmt != AV_PIX_FMT_NONE) {
            ...
            [HCDPlayerDecoder stream:fmtctx->streams[vstream] fps:&_videoFPS timebase:&_videoTimebase default:0.04];
            rotation = [HCDPlayerDecoder rotationFromVideoStream:fmtctx->streams[vstream]];
        }
    ...
    if (astream >= 0 && acodectx != NULL) {
        ...
        [HCDPlayerDecoder stream:fmtctx->streams[astream] fps:NULL timebase:&_audioTimebase default:0.025];
    ...
    if (vstream < 0 && astream < 0) {
        ...
        [HCDPlayerUtils createError:error
                         withDomain:HCDPlayerErrorDomainDecoder
                            andCode:HCDPlayerErrorCodeNoVideoAndAudioStream
                         andMessage:[HCDPlayerUtils localizedString:@"HCD_PLAYER_STRINGS_NO_VIDEO_AND_AUDIO_STREAM"]];
        return NO;
    }
...
- (NSArray<HCDPlayerVideoFrame *> *)handlePicturePacket:(AVPacket *)packet {
    ...
    HCDPlayerVideoRGBFrame *frame = [[HCDPlayerVideoRGBFrame alloc] init];
...
- (NSArray<HCDPlayerVideoFrame *> *)handleVideoPacket:(AVPacket *)packet byContext:(AVCodecContext *)context andFrame:(AVFrame *)frame andSwsContext:(struct SwsContext *)swsctx andSwsFrame:(AVFrame *)swsframe {
    ...
    do {
        ...
        if (_isYUV) {
            HCDPlayerVideoYUVFrame *yuv = [[HCDPlayerVideoYUVFrame alloc] init];
            yuv.Y = [HCDPlayerDecoder dataFromVideoFrame:frame->data[0]
                                                linesize:frame->linesize[0]
                                                   width:width
                                                  height:height];
            yuv.Cb = [HCDPlayerDecoder dataFromVideoFrame:frame->data[1]
                                                 linesize:frame->linesize[1]
                                                    width:width / 2
                                                   height:height / 2];
            yuv.Cr = [HCDPlayerDecoder dataFromVideoFrame:frame->data[2]
            ...
            HCDPlayerVideoRGBFrame *rgb = [[HCDPlayerVideoRGBFrame alloc] init];
...
- (NSArray<HCDPlayerAudioFrame *> *)handleAudioPacket:(AVPacket *)packet byContext:(AVCodecContext *)context andFrame:(AVFrame *)frame andSwrContext:(SwrContext *)swrctx andSwrBuffer:(void **)swrbuf andSwrBufferSize:(int *)swrbufsize {
    ...
    do {
        ...
        HCDPlayerAudioFrame *f = [[HCDPlayerAudioFrame alloc] init];
        f.data = mdata;
