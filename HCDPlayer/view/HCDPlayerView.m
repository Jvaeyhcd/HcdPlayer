...
@property (nonatomic, strong) HCDPlayerVideoFrame *lastFrame;
...
- (void)initProjection {
    [HCDPlayerView ortho:_mat4Projection];
}
...
- (void)createGLProgram {
    ...
    NSString *vertexShaderFilename = @"HCDPlayerVertexShader";
    if (_shouldScale) vertexShaderFilename = @"HCDPlayerRotationScaleVertexShader";
    else if (_shouldRotate) vertexShaderFilename = @"HCDPlayerRotationVertexShader";
    NSString *vertexShaderFile = [bundle pathForResource:vertexShaderFilename ofType:@"glsl"];
    GLuint vertexShader = [HCDPlayerView loadShader:GL_VERTEX_SHADER withFile:vertexShaderFile];
    NSString *fragmentShaderResource = _isYUV ? @"HCDPlayerYUVFragmentShader" : @"HCDPlayerRGBFragmentShader";
    NSString *fragmentShaderFile = [bundle pathForResource:fragmentShaderResource ofType:@"glsl"];
    GLuint fragmentShader = [HCDPlayerView loadShader:GL_FRAGMENT_SHADER withFile:fragmentShaderFile];
...
- (void)updateRotationMatrix {
    if (_shouldRotate) {
        if (_isFlipVertical) { [HCDPlayerView flipVertical:_mat3Rotation]; }
        else { [HCDPlayerView rotate:self.rotation matrix:_mat3Rotation]; }
    }
    if (_shouldScale) {
        [HCDPlayerView ratio:(GLfloat)_backingWidth/(GLfloat)_backingHeight matrix:_mat3Ratio];
        [HCDPlayerView ratio:(GLfloat)_backingHeight/(GLfloat)_backingWidth matrix:_mat3ViewRatio];
        [HCDPlayerView scale:_scale matrix:_mat3Scale];
    }
}
