...
@property (nonatomic, strong, readonly) CYCTFrameParserConfig *config;

@property (nonatomic, strong) CYCTData *drawData;
...
- (void)_considerUpdating {
    if ( 0 == _text.length && 0 == _attributedText.length ) {
        ...
        if ( _text ) _drawData = [CYCTFrameParser parserContent:_text config:_config];
        if ( _attributedText ) _drawData = [CYCTFrameParser parserAttributedStr:_attributedText config:_config];
...
- (CYCTFrameParserConfig *)__defaultConfig {
    CYCTFrameParserConfig *defaultConfig = [CYCTFrameParserConfig new];
