...
+ (id<HcdAudioManager>)audioManager {
    ...
    dispatch_once(&onceToken, ^{
        audioManager = [[HcdAudioManagerImpl alloc] init];
    });
    return audioManager;
}
...
- (BOOL)setupAudio {
    ...
    // 监听耳机的插入和拔出
    /*
     注册一个监听：sessionPropertyListener，当音频会话传递的方式（耳机/喇叭）发生改变的时候，会触发这个监听
...
- (BOOL)activateAudioSession {
    if (!_activated) {
        ...
        if (!_initialized) {
            ...
            /*
             初始化AudioSession
             这个函数在其他AudioSession函数调用之前 必须先调用一次
             */
            if (checkError(AudioSessionInitialize(NULL,
                                                  kCFRunLoopDefaultMode,
                                                  sessionInterruptionListener, // 当AudioSession中断时被调用
                                                  (__bridge void *)(self)),
...
// 当音频会话传递的方式（耳机/喇叭）发生改变的时候，会触发这个监听
// 当音量发生改变的时候
