...
static NSError * HcdmovieError (NSInteger code, id info) {
    ...
    return [NSError errorWithDomain:HcdmovieErrorDomain
                               code:code
                           userInfo:userInfo];
}
...
@implementation HcdMovieFrame
@end

@interface HcdAudioFrame()
@property (readwrite, nonatomic, strong) NSData *samples;
@end

@implementation HcdAudioFrame
- (HcdMovieFrameType) type { return HcdMovieFrameTypeAudio; }
@end

@interface HcdVideoFrame()
@property (readwrite, nonatomic) NSUInteger width;
@property (readwrite, nonatomic) NSUInteger height;
@end

@implementation HcdVideoFrame
- (HcdMovieFrameType) type { return HcdMovieFrameTypeVideo; }
@end

@interface HcdVideoFrameRGB ()
...
@implementation HcdVideoFrameYUV
- (HcdVideoFrameFormat) format { return HcdVideoFrameFormatYUV; }
@end

@interface HcdArtworkFrame()
...
@interface HcdSubtitleFrame()
@property (readwrite, nonatomic, strong) NSString *text;
@end

@implementation HcdSubtitleFrame
- (HcdMovieFrameType) type { return HcdMovieFrameTypeSubtitle; }
@end

////////////////////////////////////////////////////////////////////////////////

@interface HcdMovieDecoder () {
...
@implementation HcdMovieDecoder
...
- (void)setSelectedSubtitleStream:(NSInteger)selected {
    ...
    if (selected == -1) {
        ...
        HcdMovieError errCode = [self openSubtitleStream:subtitleStream];
        if (HcdMovieErrorNone != errCode) {
...
- (BOOL)openFile: (NSString *)path
    ...
    HcdMovieError errCode = [self openInput: path];
    
    if (errCode == HcdMovieErrorNone) {
        
        // 打开视频流
        HcdMovieError videoErr = [self openVideoStream];
        HcdMovieError audioErr = [self openAudioStream];
        
        _subtitleStream = -1;
        
        if (videoErr != HcdMovieErrorNone &&
            audioErr != HcdMovieErrorNone) {
    ...
    if (errCode != HcdMovieErrorNone) {
        ...
        if (perror)
            *perror = HcdmovieError(errCode, errMsg);
        return NO;
    }
...
- (HcdMovieError)openVideoStream:(NSInteger)videoStream {
    ...
    if (!codec)
        return HcdMovieErrorCodecNotFound;
    ...
    if (avcodec_open2(codecCtx, codec, NULL) < 0)
        return HcdMovieErrorOpenCodec;
    
    _videoFrame = av_frame_alloc();
    
    if (!_videoFrame) {
        avcodec_close(codecCtx);
        return HcdMovieErrorAllocateFrame;
    }
...
- (HcdMovieError)openSubtitleStream:(NSInteger)subtitleStream {
    ...
    if (codecDesc && (codecDesc->props & AV_CODEC_PROP_BITMAP_SUB)) {
        ...
        return HcdMovieErroUnsupported;
    }
    ...
    if (codecCtx->subtitle_header_size) {
        ...
        if (s.length) {
            ...
            NSArray *fields = [HcdMovieSubtitleASSParser parseEvents:s];
...
- (HcdVideoFrame *)handleVideoFrame {
    ...
    if (_videoFrameFormat == HcdVideoFrameFormatYUV) {
        
        HcdVideoFrameYUV * yuvFrame = [[HcdVideoFrameYUV alloc] init];
        ...
        HcdVideoFrameRGB *rgbFrame = [[HcdVideoFrameRGB alloc] init];
...
- (HcdAudioFrame *)handleAudioFrame{
    ...
    HcdAudioFrame *frame = [[HcdAudioFrame alloc] init];
...
- (HcdSubtitleFrame *)handleSubtitle:(AVSubtitle *)pSubtitle {
        ...
        if (rect) {
            ...
            if (rect->text) { // rect->type == SUBTITLE_TEXT
                ...
                if (s.length) {
                    ...
                    NSArray *fields = [HcdMovieSubtitleASSParser parseDialogue:s numFields:_subtitleASSEvents];
                    if (fields.count && [fields.lastObject length]) {
                        
                        s = [HcdMovieSubtitleASSParser removeCommandsFromEventText: fields.lastObject];
                        if (s.length) [ms appendString:s];
                    }
                }
            }
    ...
    HcdSubtitleFrame *frame = [[HcdSubtitleFrame alloc] init];
    frame.text = [ms copy];
...
- (BOOL)setupVideoFrameFormat:(HcdVideoFrameFormat)format {
    if (format == HcdVideoFrameFormatYUV &&
        ...
        _videoFrameFormat = HcdVideoFrameFormatYUV;
        return YES;
    }
    
    _videoFrameFormat = HcdVideoFrameFormatRGB;
        ...
        if (packet.stream_index ==_videoStream) {
            ...
            while (pktSize > 0) {
                ...
                if (gotframe) {
                    ...
                    HcdVideoFrame *frame = [self handleVideoFrame];
                    if (frame) {
            ...
            while (pktSize > 0) {
                ...
                if (gotframe) {
                    ...
                    HcdAudioFrame * frame = [self handleAudioFrame];
                    if (frame) {
            ...
            if (packet.size) {
                ...
                HcdArtworkFrame *frame = [[HcdArtworkFrame alloc] init];
            ...
            while (pktSize > 0) {
                ...
                if (gotsubtitle) {
                    ...
                    HcdSubtitleFrame *frame = [self handleSubtitle: &subtitle];
                    if (frame) {
                        [result addObject:frame];
                    }
